...
using OmniSteeringCommandListener = romea::CommandListener<romea::OmniSteeringCommand>;
...
public :
  ...
  void init(const std::string & joystick_type)
  {
    ...
    cmd_sub = std::make_unique<OmniSteeringCommandListener>(
  ...
  std::unique_ptr<OmniSteeringCommandListener> cmd_sub;
...
TEST_F(TestOmniSteeringTeleop, testSlowModeXbox)
{
  ...
  EXPECT_DOUBLE_EQ(cmd_sub->get_command().lateralSpeed,3.0);
  EXPECT_DOUBLE_EQ(cmd_sub->get_command().angularSpeed,5.0);
}
...
TEST_F(TestOmniSteeringTeleop, testTurboModeXbox)
{
  ...
  EXPECT_DOUBLE_EQ(cmd_sub->get_command().lateralSpeed,-4.0);
  EXPECT_DOUBLE_EQ(cmd_sub->get_command().angularSpeed,-6.0);
}
...
TEST_F(TestOmniSteeringTeleop, testNoCmdXbox)
{
  ...
  EXPECT_DOUBLE_EQ(cmd_sub->get_command().lateralSpeed,0.0);
...
TEST_F(TestOmniSteeringTeleop, testSlowModeDualshock4)
{
  ...
  EXPECT_NEAR(cmd_sub->get_command().lateralSpeed,1.5,0.001);
  EXPECT_NEAR(cmd_sub->get_command().angularSpeed,2.5,0.001);
}
...
TEST_F(TestOmniSteeringTeleop, testTurboModeDualshock4)
{
  ...
  EXPECT_NEAR(cmd_sub->get_command().lateralSpeed,-2.0,0.001);
  EXPECT_NEAR(cmd_sub->get_command().angularSpeed,-3.0,0.001);
}
