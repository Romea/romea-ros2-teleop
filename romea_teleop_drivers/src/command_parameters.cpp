...
{
  romea::declare_parameter_with_default<int>(node, COMMAND_MESSAGE_PRIORITY_PARAM_NAME, -1);
}

//-----------------------------------------------------------------------------
int get_command_output_message_priority(std::shared_ptr<rclcpp::Node> node)
{
  return romea::get_parameter<int>(node, COMMAND_MESSAGE_PRIORITY_PARAM_NAME);
}
